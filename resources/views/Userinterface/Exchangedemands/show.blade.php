...
<h3>object information</h3>
        ...
        <div class="card-body">
            <div class="image-block">
                ...
                @if (!empty($exchange->picture))
                    @if (str_starts_with($exchange->picture, 'http') || str_starts_with($exchange->picture, 'https://'))
                        <img src="{{ $exchange->picture }}" alt="blog-img" class="img-fluid rounded w-100" loading="lazy">
                    @else
            ...
            <div class="blog-description mt-3">
                ...
                <span class="badge bg-warning my-2  " href="?status={{ $exchange->status }}">{{ $exchange->status }}</span>
            @elseif ($exchange->status === 'accepted')
                <span class="badge bg-success my-2" href="?status={{ $exchange->status }}">{{ $exchange->status }}</span>
            @else
                <span class="badge bg-danger my-2" href="?status={{ $exchange->status }}">{{ $exchange->status }}</span>
            @endif
